package mid2.generic.test.ex2;

import mid2.generic.test.ex2.unit.Marine;
import mid2.generic.test.ex2.unit.Zealot;

public class UnitUtilTest {
    public static void main(String[] args) {
        Marine m1 = new Marine("마린1", 50);
        Marine m2 = new Marine("마린2", 40);
        Marine resultMarine = UnitUtil.maxHp(m1,m2);
        System.out.println("resultMarine = " + resultMarine);

        Zealot z1 = new Zealot("질럿1", 50);
        Zealot z2 = new Zealot("질럿2", 40);
        Zealot resultZealot = UnitUtil.maxHp(z1,z2);
        System.out.println("resultZealot = " + resultZealot);
    }
}
